...
Например: 
Необходимо сдвинуть (опустить) мобильное меню на высоту шапки. Высота шапки меняеться в зависимости от размеров экрана устройства (device). Для этого в файле js узнаем высоту шапки и записываем в переменную таблицу стилей: 
```
const headerHeight = header.offsetHeight;
document.querySelector(':root').style.setProperty('--header-height', `${headerHeight}px`);
```
А в таблице стилей значение записываем так:
```
.menu {
